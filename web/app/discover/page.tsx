...
export default function DiscoverPage() {
  ...
  async function loadUser() {
    try {
      ...
      if (user) {
        setUserId(user.id);
        const favsRes = await fetch("/api/favorites?target_type=case");
  ...
  async function toggleFavorite(caseId: string) {
    ...
    if (isFav) {
      await fetch("/api/favorites", {
        ...
        body: JSON.stringify({ target_type: "case", target_id: caseId }),
      });
  ...
  const filteredCases = useMemo(() => {
    ...
    if (activeCategory !== "all") {
      result = result.filter((c) =>
        c.slug?.includes(activeCategory) || c.title.toLowerCase().includes(activeCategory)
      );
    }
    ...
    switch (sort) {
      case "popular":
        result = [...result].sort((a, b) => b.sales_count - a.sales_count);
        break;
      case "most_favorited":
        result = [...result].sort((a, b) => b.sales_count - a.sales_count);
        break;
            ...
            <div className="inspo-sidebar-section">
              ...
              <ul className="inspo-nav-list">
                ...
                >
                  ...
                  我的收藏
                  {favorites.size > 0 && <span className="inspo-badge">{favorites.size}</span>}
                </li>
              </ul>
            </div>
          </>
      ...
      <div className="inspo-main">
        <div className="inspo-topbar">
          <div className="inspo-topbar-left">
            <h2 className="inspo-page-title">
              {CATEGORIES.find((c) => c.slug === activeCategory)?.label || "发现"}
            </h2>
          </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-text">暂无案例</div>
          </div>
        ) : (
