...
export default function ShopPage() {
    ...
    try {
      ...
      if (userRes.user) {
        ...
        const [favsRes, partsRes] = await Promise.all([
          fetch("/api/favorites?target_type=product").then((r) => r.json()),
          fetch("/api/user-parts").then((r) => r.json()),
        ]);
  ...
  const filteredParts = useMemo(() => {
    ...
    if (activeCategory === "my-electronics") {
      const eIds = getCategoryIdsBySlug("electronics");
      result = result.filter((p) => p.product && eIds.includes(p.product.category_id));
    } else {
      const mIds = getCategoryIdsBySlug("mechanical");
      result = result.filter((p) => p.product && mIds.includes(p.product.category_id));
    }
    if (search.trim()) {
      const q = search.toLowerCase();
      result = result.filter((p) => p.product?.name.toLowerCase().includes(q));
    }
    return result;
    ...
    <div className="page-content">
      ...
      <div className="inspo-main">
        <div className="inspo-topbar">
          <div className="inspo-topbar-left">
            <h2 className="inspo-page-title">
              {isMyPartsView
                ? activeCategory === "my-electronics" ? "我的电子模块" : "我的机械零件"
                : "零件商城"}
            </h2>
          </div>
          <div className="inspo-topbar-right">
            <div className="inspo-search-wrap">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
              <input type="text" className="inspo-search" placeholder="搜索零件..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            {!isMyPartsView && (
              <div className="inspo-sort">
                <span>排序</span>
                <select className="inspo-sort-select" value={sort} onChange={(e) => setSort(e.target.value as SortOption)}>
        ...
        {loading ? (
          ...
          filteredParts.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-text">暂无零件</div>
            </div>
          ) : (
          ...
          <div className="empty-state">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/></svg>
