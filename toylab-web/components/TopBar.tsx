...
const NAV_ITEMS = [
  ...
  { href: "/magic", label: "MakeX赛事", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/></svg> },
  { href: "/projects", label: "社区", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3 20c0-3.3 2.7-6 6-6s6 2.7 6 6"/><circle cx="18" cy="10" r="2.5"/><path d="M15.5 20c0-2.5 1.1-4.5 2.5-4.5s2.5 2 2.5 4.5"/></svg> },
];
...
export default function TopBar() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      apiFetch("/auth/me")
        .then((r) => r.json())
        .then(({ user }) => { if (!cancelled && user) setProfile(user as Profile); })
        .catch(() => {});
    };
  ...
  return (
    <header className="top-bar">
      ...
      <div className="top-bar-right">
        ...
        {profile ? (
          <span className="topbar-icon-wrap">
            <button type="button" className="user-avatar-btn" style={{ background: profile.avatar_color || "#7C3AED" }}
              onClick={handleSignOut} title={`${profile.nickname || profile.username} — 点击退出`}>
